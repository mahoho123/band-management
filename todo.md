...
## 新功能 (114) - 改進時間選擇 UI（修正版 - 清晰分離模式）
- [x] 重新設計 TimeSelector 組件：清晰分離「具體時間」和「時間段」兩種模式
- [x] 具體時間模式：時 + 分 + 上午/下午（全部必填）
- [x] 時間段模式：四個按鈕「待定」/「上午」/「下午」/「晚上」（必選其一，不可留空）
- [x] 更新 Home.tsx 整合新的 TimeSelector
- [x] 測試新 UI 確保時間段不可留空
- [ ] 保存 checkpoint

## 新功能 (115) - 刪除時間段功能
- [x] 刪除時間段模式（待定/上午/下午/晚上）
- [x] 簡化 TimeSelector 組件：只保留具體時間輸入
- [x] 更新 Home.tsx 中的時間選擇狀態管理
- [x] 移除時間段相關的驗證邏輯
- [ ] 保存 checkpoint
