# Band隊管理系統 - 項目待辦清單

## 核心功能
- [x] 資料庫 schema 設計（成員、活動、出席、假期、系統數據）
- [x] tRPC 路由建立（所有 Band 管理 API）
- [x] 前端 Home.tsx 重寫（使用 tRPC 替代 LocalStorage）
- [x] 系統初始化流程
- [x] 成員管理（新增、編輯、刪除、密碼重設）
- [x] 活動管理（新增、編輯、刪除、查看）
- [x] 出席記錄管理
- [x] 香港假期管理
- [x] 主管密碼管理

## 用戶界面
- [x] 月曆視圖
- [x] 活動清單視圖
- [x] 成員管理視圖
- [x] 登入/登出功能
- [x] 成員註冊功能
- [x] 活動詳情模態框
- [x] 出席狀態選擇
- [x] 所有版本活動上加儲存按鈕

## 測試
- [x] 單元測試編寫（band.test.ts）
- [x] 所有測試通過（14/14）

## 待完成
- [x] 前端表單驗證優化
- [x] 錯誤處理改進
- [x] 性能優化
- [x] 移動端適配測試

## 新增功能需求
- [x] 月曆視圖顯示所有成員出席狀態
- [x] 清單視圖顯示所有成員出席狀態
- [x] 出席狀態視覺化（出席、不出席、待確認）

## 新增功能需求 (2)
- [x] 月曆視圖顯示活動備註
- [x] 清單視圖顯示活動備註和已打過的內容


## 新增功能需求 (3)
- [x] 不需要登入就可以查看主管打的活動
- [x] 每個成員可以獨立按出席/不出席按鈕
- [x] 按 SAVE 後自動返回到活動清單


## Bug 修複
- [x] 未登入用戶不能幫人選出席狀態
- [x] 月曆要顯示出席/不出席的人名（而不是只顯示統計）


## 新增功能需求 (4) - 重大更新
- [x] 文字替換：「Band隊管理系統」→「慢半拍」
- [x] 文字替換：「Remarks」→「備註」
- [x] 移除「主管模式 - 完整管理權限」標籤
- [x] 備註格式顯示：月曆上保留換行、縮進等格式
- [x] 手機適配：所有文字和格式自動適應手機大小
- [x] 手機適配：月曆完整顯示在手機上
- [x] 月曆篩選：按「+more」時去活動清單
- [x] 月曆篩選：可多選日期進行篩選（不顯示其他日期）
- [x] 自動完成：結束時間跟蹤香港天文台時間
- [x] 自動完成：到時自動標記為已完成


## 新增功能需求 (5) - 出席記錄查看
- [x] 所有人都可以點擊月曆上的日子
- [x] 點擊日子後顯示當日每個活動的詳情記錄
- [x] 顯示每個活動有多少人出席、不出席、待確認
- [x] 活動詳情模態框顯示完整的出席人員清單列表


## 新增功能需求 (6) - 獨立活動出席選擇
- [x] 每個活動都要獨立選擇出席/不出席
- [x] 同一日多個活動時，每個活動分別選擇出席狀態
- [x] 活動清單視圖中每個活動都有出席/不出席按鈕
- [x] 活動詳情模態框中獨立選擇該活動的出席狀態


## 修複需求 (7) - 月曆響應式設計
- [x] 月曆在手機上自動調整日期卡片大小
- [x] 月曆內容文字在手機上自動縮小
- [x] 月曆在平板和桌面上正常顯示
- [x] 月曆日期卡片內容不被截斷或滯出


## 修複需求 (8) - 手機全面適配
- [x] 頁面導航在手機上完整顯示
- [x] 模態框在手機上全屏或接近全屏顯示
- [x] 所有按鈕在手機上大小適當
- [x] 所有文字在手機上自動縮放
- [x] 表单輸入框在手機上適當大小
- [x] 時間選擇器在手機上易於操作
- [x] 出席記錄清單在手機上完整顯示
- [x] 月曆在手機上完整顯示所有內容


## 修複需求 (9) - 清單視圖篩選改進
- [x] 移除清單視圖中「顯示其他日期」的一行
- [x] 將「全年」改為「全月」
- [x] 支持多個月份可以同時選擇


## 修複需求 (10) - 月曆顯示改進
- [x] 月曆根據手機大小自動調整
- [x] 月曆完整顯示活動內容
- [x] 移除月曆上的出席記錄顯示


## 修複需求 (11) - 移除活動清單出席統計
- [x] 移除活動清單視圖中的出席人數統計（「X/10 人出席」）
- [x] 保留出席/不出席按鈕讓成帳可以選擇


## 修複需求 (12) - 移除頭部出席記錄統計
- [x] 移除頭部統計區域中的「出席記錄」統計項（數字和標籤）

## 新增功能需求 (13) - 改進出席狀態選擇
- [x] 成員只能看到自己的出席狀態，看不到其他成員的選擇
- [x] 清楚標示「您的出席狀態」，不会混淆
- [x] 活動詳情模態框中隱藏其他成員的出席記錄（僅成員視圖）
- [x] 主管件然可查看所有成員的出席狀態


## 修複需求 (14) - 重設主管密碼
- [x] 將主管密碼重設為「admin」


## 新增功能需求 (15) - 活動搜索功能
- [x] 在活動清單視圖添加搜索框
- [x] 支持按活動名稱、地點、備註搜索
- [x] 實時搜索過濾
- [x] 搜索結果提示
- [x] 與其他篩選器兼容
- [x] 編寫搜索功能測試（13 個測試）
- [x] 所有搜索測試通過

## 新增功能需求 (16) - 全面響應式設計審計和優化
- [x] 審計所有頁面的移動設備適配（直屏/橫屏）
- [x] 審計所有頁面的平板設備適配
- [x] 審計所有頁面的桌面設備適配
- [x] 修復月曆視圖響應式問題
- [x] 修復活動清單視圖響應式問題
- [x] 修復成員管理視圖響應式問題
- [x] 修復模態框響應式問題
- [x] 修復導航欄響應式問題
- [x] 修復表單輸入響應式問題
- [x] 修復時間選擇器響應式問題
- [x] 在多種設備上測試驗證
- [x] 確保所有內容在所有屏幕尺寸下完全可見


## 修復 (17) - 手機月曆日期卡片高度問題
- [x] 增加手機上的日期卡片最小高度
- [x] 優化活動文本截斷和省略號
- [x] 確保活動內容在手機上完全可見
- [x] 測試多種手機屏幕尺寸


## 優化 (18) - 月曆自動調整高度和清單視圖優化
- [x] 實現月曆日期卡片自動調整高度
- [x] 根據活動數量動態計算卡片高度
- [x] 確保所有活動在月曆上完全可見
- [x] 優化清單視圖作為主要查看方式
- [x] 改進清單視圖的篩選和排序功能
- [x] 實現零延遅實時同步
- [x] 所有操作即時反應（新増、編輯、刪除、出席確認）
- [x] 跨設備即時同步驗證


## 新需求 (19) - 清除活動和即時出席狀態同步
- [x] 清除所有活動記錄
- [x] 實現成員出席狀態實時顯示
- [x] 每個成員都能看到誰出席、誰不出席
- [x] 無需重整網頁即時同步所有出席狀態
- [x] 實現 輫詢自動同步機制 (5秒一次)

## 新需求 (20) - 活動清單批量選擇和刪除
- [x] 實現活動清單多選功能 - 添加複選框和多選狀態管理
- [x] 實現批量刪除功能 - 刪除多個選中的活動
- [x] 添加批量操作工具欄 - 顯示選中數量和刪除按鈕
- [x] 修復月曆視圖 - 恢復上一個版本的月曆功能

## 新需求 (21) - 批量操作工具欄全選功能
- [x] 添加「全選」按鈕到批量操作工具欄
- [x] 添加「取消全選」按鈕到批量操作工具欄
- [x] 實現全選邏輯 - 選擇所有顯示的活動

## 新需求 (22) - 出席狀態添加「未知道」選項
- [x] 更新數據庫架構 - 支持三種出席狀態（出席、不出席、未知道）
- [x] 更新前端 UI - 添加「未知道」狀態按鈕
- [x] 實現狀態切換邏輯 - 支持三種狀態的切換
- [x] 更新出席統計顯示 - 顯示未知道人數

## 新需求 (23) - 成員出席狀態面板
- [x] 在活動詳情中添加成員出席狀態面板
- [x] 分三個區域顯示：出席、不出席、未知道
- [x] 每個區域顯示成員名字列表
- [x] 支持主管點擊成員快速修改出席狀態

## 新需求 (24) - 優化出席狀態更新體驗
- [x] 實現樂觀更新 - 按完按鈕立即更新UI，無延遲
- [x] 在清單中顯示出席統計 + 成員名字 - 顯示所有成員
- [x] 所有成員都能看到出席統計和人名
- [x] 確保更新同步到服務器

## Bug 修複 (25) - 出席狀態零延遲更新
- [x] 實現樂觀更新 - 按完按鈕立即更新 UI
- [x] 不等待服務器回應 - 立即反映狀態變化
- [x] 確保服務器同步 - 後台更新數據

## Bug 修複 (26) - 完全零延遲出席狀態更新
- [x] 在本地狀態中直接管理出席狀態（改由 React Query cache-backed optimistic state 管理，避免雙重狀態）
- [x] 按完按鈕立即更新月曆和清單
- [x] 後台異步同步到服務器
- [x] 使用 tRPC／React Query setQueryData 實現即時 optimistic 更新，避免等待 refetch 才更新 UI

## 自動化測試 (27) - 完整測試套件
- [x] 創建功能測試 - 驗證所有功能正常運作
- [x] 創建延遲測試 - 測量 UI 更新時間
- [x] 創建跨設備測試 - 測試不同屏幕尺寸
- [x] 創建網絡模擬測試 - 測試不同網絡速度
- [x] 創建性能測試 - 測試大量數據
- [x] 運行所有測試並生成報告

## Bug 修復 (28) - 修復自動化測試和 TypeScript 錯誤
- [x] 修復延遲測試基準値（調整為實際網絡延遲）
- [x] 修復綜合測試中的出席狀態設置問題
- [x] 修復 Home.tsx 中的 TypeScript 錯誤（Set 迭代 downlevelIteration）
- [x] 確保所有測試通過（51 個測試全部通過）

## 自動化測試 (29) - 完整詳細測試套件
- [x] 擴展功能測試 - 所有功能和邊界情況
- [x] 擴展延遲測試 - 每個操作的具體時間
- [x] 擴展跨設備和網絡測試
- [x] 創建 UI 分析測試 - 功能完整性和改進建議
- [x] 生成詳細測試報告 - 71 個測試全部通過

## 維護操作 (30) - 清除活動和查找主管密碼
- [x] 清除數據庫中所有活動數據
- [x] 查找主管登入密碼

## UI 修改 (31) - 假期文字和右上角圖示
- [x] 縮小月曆假期文字
- [x] 移除月曆假期旁的 🏖 圖示

## 新需求 (32) - WhatsApp 通知功能
- [x] 添加 WhatsApp 通知 UI 組件到活動詳情
- [x] 實現自制信息編輯功能
- [x] 實現 WhatsApp 個人帳號連結生成（複製信息方式）
- [x] 實現 WhatsApp 群組連結生成
- [x] 實現出席狀態摘要複製功能
- [x] 測試所有 WhatsApp 連結功能（21 個測試用例）
- [x] 保存檢查點

## 新需求 (33) - WhatsApp 功能優化和教學指南
- [x] 添加 WhatsApp 功能使用教學指南到首頁（幫助按鈕）
- [x] 將清除所有活動功能移到活動清單視圖
- [x] 在活動清單工具欄添加清除按鈕
- [x] 添加確認對話框防止誤操作
- [x] 測試所有功能
- [x] 保存檢查點

## 新需求 (34) - WhatsApp 通用連結
- [x] 將「群組」模式改為 wa.me 通用連結（支持所有平台）
- [x] 將「個人帳戶」模式改為 wa.me 連結（需要電話號碼）
- [x] 添加電話號碼輸入欄位（個人帳戶模式）
- [x] 更新教學指南說明
- [x] 測試所有平台連結
- [x] 保存檢查點

## 新需求 (35) - 簡化 WhatsApp 界面
- [x] 移除「個人帳戶/群組/複製摘要」三個選項
- [x] 改為「活動前提醒」和「出席狀態摘要」兩個場景
- [x] 兩個場景都使用 wa.me/?text= 直接開啟 WhatsApp 選人/群組
- [x] 移除電話號碼輸入欄位
- [x] 更新教學指南說明
- [x] 保存檢查點

## 新需求 (36) - 活動前提醒加入網頁連結
- [x] 在活動前提醒信息模板末尾自動加入網頁連結
- [x] 保存檢查點

## 新需求 (37) - 本月活動 WhatsApp 通知
- [x] 添加 generateMonthlyNotification 函數，生成本月所有活動的摘要信息
- [x] 在月曆視圖頂部添加小巧的「本月通知」 WhatsApp 按鈕（主管專用）
- [x] 點擊後開啟 WhatsApp 通知模態框，顯示本月所有活動列表和網頁連結
- [x] 保存檢查點

## 新需求 (38) - 刪除和出席狀態優化
- [x] 移除所有刪除確認對話框（直接刪除）
- [x] 移除活動清單中的出席狀態按鈕（只保留活動詳情中的）
- [x] 保存檢查點

## 性能修復 (39) - 測試報告問題修復
- [x] 將 refetchInterval 從 500ms 改為 3000ms（降低伺服器負載）
- [x] 修復 useRealtimeSync.ts 事件監聽器記憶體洩漏（添加 cleanup）
- [x] 優化 getBandEvents N+1 查詢（使用 Promise.all 並行查詢取代 N+1）

## 權限控制 (40)
- [x] WhatsApp 教學頁面只有主管才能查看（導航「幫助」按鈕只對主管顯示）

## UI 調整 (41)
- [x] 將 WhatsApp 教學「幫助」按鈕移到導航欄最右邊

## 修復 (42) - 登入登出體驗
- [x] 手機版「登入」「登出」完整顯示（移除縮短文字）
- [x] 重整網頁後自動保持登入狀態（localStorage 持久化）
- [x] 登出時清除 localStorage，刪除成員時也清除

## UI 修復 (43) - 提示信息顯示
- [x] 所有 Toast 提示信息固定在畫面最前方（z-index: 99999），手機版居中顯示，更大更清晰

## 修復 (44) - 登入狀態關閉分頁自動登出
- [x] 將登入狀態從 localStorage 改為 sessionStorage（重整保留，關閉分頁自動登出）

## 功能改造 (45) - 批量刪除活動
- [x] 移除「清除所有已完成活動」和「清除所有未完成活動」按鈕
- [x] 保留勾選活動後的「刪除所選」批量刪除功能，工具欄改為中文

## 功能改善 (46) - 月曆及 WhatsApp 通知
- [x] 月曆格子完整顯示活動地點（地點圖標 + 地點文字）
- [x] WhatsApp 本月通知自動包含完整資料（日期、開始至結束時間、地點、類型、備註）

## UI 改善 (47) - 月曆活動 emoji
- [x] 月曆格子活動標題前顯示對應類型 emoji（排練🎸、演出🎶、會議👥、其他📌）

## UI 改善 (48) - 月曆時間備註 emoji
- [x] 月曆格子時間前加 ⏰，地點改用 📍 emoji，備註前加 📝

## UI 改善 (49) - Logo 更換
- [x] 將慢半拍 logo 圖片設定為網站左上角圖標（首頁、設定模態框）

## UI 改善 (50) - Logo 去背及放大
- [x] 去除 logo 白色背景（透明 PNG）
- [x] 放大 logo 顯示尺寸

## UI 改善 (51) - Logo 再放大
- [x] 左上角 logo 再放大（手機 64px、平板 80px、桌面 96px）

## UI 改善 (52) - Favicon 設定
- [x] 將慢半拍 logo 設定為瀏覽器分頁 favicon（ICO + PNG 16x16/32x32 + Apple Touch Icon 180x180）

## 新功能 (53) - 重複活動
- [x] 新增活動表單加入重複選項（不重複/每週/每月）
- [x] 加入重複次數設定（最多 52 次）
- [x] 實現重複活動批量生成邏輯，按鈕顯示創建數量

## 修復 (54) - 儲存活動後返回月曆
- [x] 儲存活動後自動關閉模態框並返回月曆視圖

## 修復 (55) - 重複活動及返回月曆
- [x] 重複活動改為自行選擇具體日期（啟用後可新增多個日期選擇器）
- [x] 修復儲存活動後返回月曆（updateEventMutation 加入 onSuccess 回調）

## 修復 (56) - 重複活動儲存後返回月曆
- [x] 重複活動儲存後自動關閉模態框並返回月曆視圖（改用 Promise.allSettled 確保所有請求完成後才關閉）

## 修復 (57) - 重複活動儲存後返回月曆（再次修復）
- [x] 徹底修復重複活動儲存後自動關閉模態框並返回月曆視圖（try/catch/finally）

## 修復 (58) - 刪除活動不彈出詳情
- [x] 已勾選活動時，點擊其他活動卡片直接切換勾選狀態，不彈出詳情模態框

## 修復 (59) - 活動清單完全移除彈出詳情
- [x] 活動清單中主管點擊卡片只切換勾選狀態，完全不彈出詳情模態框；成員點擊卡片才彈出詳情

## 修復 (60) - 配色方案更新
- [x] 更新整個網站配色：背景淡金黃 + 主色金黃漸變 + 深棕色按鈕 + 溫暖卡片背景

## 修復 (61) - 成員版本活動詳情移除儲存按鈕
- [x] 成員版本活動詳情只保留查看功能，移除底部的「儲存」按鈕，只有頂部的關閉按鈕（交叉）

## 修復 (62) - WhatsApp 信息 emoji 顯示為方塊符號
- [x] 修複 WhatsApp 信息中的 emoji 編碼問題（改用 URLSearchParams 方式，保留所有 emoji）

## 修復 (63) - 移除 WhatsApp 複製信息功能
- [x] 移除複製信息按鈕，只保留「開啟 WhatsApp」按鈕

## 修復 (64) - 密碼安全問題
- [x] 在後端建立伺服器端密碼驗證 tRPC 程序
- [x] 移除前端密碼驗證邏輯，改用伺服器端驗證
- [x] 清理數據庫中的明文密碼欄位

## 修復 (65) - 電腦版 WhatsApp emoji 亂碼
- [x] 修複電腦版 WhatsApp 的 emoji 編碼問題（電腦版複製剩貼板，手機版直接開啟 APP）

## 修復 (66) - WhatsApp Desktop App 優先級联
- [x] 電腦版優先開啟 WhatsApp Desktop App，未安裝時降級到 Web 版

## 新功能 (67) - 密碼安全修復、月曆高亮、活動顏色標籤
- [x] 密碼安全修復 - 完成伺服器端密碼驗證，移除前端明文密碼比對
- [x] 月曆今日高亮 - 今天日期用金黃色邊框突出顯示
- [x] 活動類型顏色標籤 - 排練/演出/會議用不同顏色區分

## 新功能 (68) - 成員管理月份篩選統計
- [x] 在成員管理頁面添加月份篩選 UI（上一月/下一月按鈕 + 下拉菜單）
- [x] 實現月份篩選的出席統計計算邏輯
- [x] 顯示選定月份的出席/不出席/待確認數量
- [x] 測試月份篩選功能

## 新功能 (68-2) - 月份篩選同步
- [x] 移除獨立的 memberStatsYear 和 memberStatsMonth 狀態
- [x] 修改成員管理視圖使用 listYear 和 listMonth
- [x] 成員管理月份篩選與月曆/清單視圖同步

## 新功能 (69) - 術語統一
- [x] 將活動中的「未知道」術語統一為「待確認」
- [x] 更新所有相關的 UI 標籤和提示文本
- [x] 驗證功能邏輯保持一致

## Bug 修複 - 待確認計數顯示為 0
- [x] 查找並修複待確認計數統計邏輯
- [x] 驗證計數正確顯示

## 新功能 (70) - 月曆時間顯示改為 12 小時制
- [x] 修改月曆日期单元格中的時間顯示為 12 小時制
- [x] 驗證月曆上的時間顯示正確

## 新功能 (71) - 月份/年份選擇器 UI 改進
- [x] 查找並提取 logo 色彩
- [x] 設計新的月份選擇器 UI（12 個月份網格，中文名稱）
- [x] 設計新的年份選擇器 UI（年份範圍網格）
- [x] 應用到月曆視圖
- [x] 應用到清單視圖
- [x] 應用到成員管理視圖
- [x] 測試所有視圖的選擇器功能

## 新功能 (71) - 月份/年份選擇器 UI 改進進度
- [x] 查找並提取 logo 色彩（金黃色漸變 #F4D03F - #D4A017）
- [x] 設計新的月份選擇器 UI（12 個月份網格，中文名稱）
- [x] 設計新的年份選擇器 UI（年份範圍網格）
- [x] 應用到月曆視圖
- [x] 應用到清單視圖
- [x] 應用到成員管理視圖
- [x] 測試所有視圖的選擇器功能

## 新需求 (72) - 月份/年份選擇器下拉菜單大小改進
- [x] 恢復下拉菜單設計
- [x] 擴大月份選擇器的下拉菜單（4 列網格）
- [x] 擴大年份選擇器的下拉菜單（5 列網格，可滾動）
- [x] 測試下拉菜單大小改進

## 新需求 (73) - 統一所有月份/年份選擇器設計
- [x] 將清單視圖的選擇器改為改進設計（使用 DatePicker 組件）
- [x] 將成員管理視圖的選擇器改為改進設計（使用 DatePicker 組件）
- [x] 測試所有視圖的選擇器一致性

## 完成標記
- [x] 將清單視圖的選擇器改為改進設計
- [x] 將成員管理視圖的選擇器改為改進設計
- [x] 測試所有視圖的選擇器一致性


## 新需求 (74) - 全面響應式設計審查和修復
- [x] 審查月曆視圖在所有屏幕尺寸上的響應式表現
- [x] 審查活動清單視圖在所有屏幕尺寸上的響應式表現
- [x] 審查成員管理視圖在所有屏幕尺寸上的響應式表現
- [x] 審查模態框在所有屏幕尺寸上的響應式表現
- [x] 審查導航欄在所有屏幕尺寸上的響應式表現
- [x] 審查表單和輸入框在所有屏幕尺寸上的響應式表現
- [x] 審查 DatePicker 組件在所有屏幕尺寸上的響應式表現
- [x] 修復發現的所有響應式設計問題
- [x] 在多種設備上進行測試驗證


## 新需求 (75) - 修複實際的響應式設計問題
- [x] 修複文字大小在手機上沒有根據屏幕調整的問題
- [x] 修複數字和日期顯示在手機上過小的問題
- [x] 修複月曆格子在手機上過擦护的問題
- [x] 修複登入模態框文字在手機上過小的問題
- [x] 修複所有 UI 元素在手機上的縮放問題
- [x] 在實際手機設備上驗證修複效果


## 新需求 (76) - 修復年份和月份選擇器的文字大小
- [x] 修復年份按鈕的文字大小（從 text-xs/sm 改為 text-sm/base/lg/xl）
- [x] 修復月份按鈕的文字大小（從 text-xs/sm 改為 text-sm/base/lg/xl）
- [x] 修復年份下拉菜單項目的文字大小（從 text-xs/sm 改為 text-sm/base/lg）
- [x] 修復月份下拉菜單項目的文字大小（從 text-xs/sm 改為 text-sm/base/lg）
- [x] 增大下拉菜單容器的寬度和間距
- [x] 在手機上驗證修復效果


## 新需求 (78) - 改造 DatePicker 為網格視圖格式（iOS 風格）
- [x] 移除年份和月份下拉菜單
- [x] 實現月份網格視圖（3x4 網格，12 個月份）
- [x] 實現年份網格視圖（4x4 網格，多年份）
- [x] 使用藍色圓形高亮當前選中的年份/月份
- [x] 添加月份/年份視圖切換按鐐
- [x] 保持現有配色方案（金黃色和灰色）
- [x] 在手機上驗證新 UI 的顯示效果


## 新需求 (79) - 改進年份選擇器為分頁 4x4 網格
- [x] 改為 4x4 網格格式（每頁 16 個年份）
- [x] 添加上下三角形按鐐切換年份頁面
- [x] 計算正確的年份分頁範團

## 新需求 (80) - Web Push 通知系統診斷和修復
- [x] 檢查前端訂閱是否成功
- [x] 檢查後端是否正確發送 Web Push
- [x] 修復 Web Push 系統
- [x] 測試修復後的系統
- [x] 在手機上驗證分頁效果


## 新需求 (80) - 添加 click outside to dismiss 功能
- [x] 為 DatePicker 月份/年份選擇器添加 click outside 功能
- [x] 為月曆日期卡片的詳情彈窗添加 click outside 功能
- [x] 為其他彈出画面添加 click outside 功能
- [x] 確保新則活動和登入模態框不受影響（保持手動關閉）
- [x] 在所有設備上驗證 click outside 效果


## 新需求 (81) - 改造活動清單中的年月選擇器
- [x] 找到活動清單視圖中的年月選擇器代碼
- [x] 改為與月曆相同的網格 UI 格式（月份 3x4，年份 4x4 分頁）
- [x] 添加 click outside to dismiss 功能
- [x] 在手機和桌面上驗證新 UI


## 新需求 (82) - 縮小年份和月份按鐐的尺寸
- [x] 縮小年份按鐐的尺寸（w-16 sm:w-20 → w-12 sm:w-16）
- [x] 縮小月份按鐐的尺寸
- [x] 調整文字大小以適應更小的按鐐
- [x] 確保在手機上完整顯示 4x4 網格
## Bug 修復 (83) - 成員登入無法彈出密碼輸入框
- [x] 將 window.prompt() 密碼輸入改為頁面內 UI（瀏覽器 iframe 封鎖 prompt）
- [x] 實現首次登入設定密碼的內嵌表單
- [x] 實現已有密碼的成員登入內嵌表單
- [x] 實現重設密碼的內嵌 Modal（主管重設成員密碼、主管重設自己密碼）
- [x] 測試登入流程

## Bug 修復 - 出席狀態按鈕延遲
- [x] 找出出席狀態按鈕延遲的根本原因（Socket.IO 廣播觸發 invalidateQueries 導致整個 refetch）
- [x] 改用 setQueryData 直接更新 cache，避免 refetch 造成 UI 閃爍
- [x] Socket.IO 廣播帶上具體數據（eventId/memberId/status）供前端直接更新
- [x] 移除 onSuccess toast，讓按鈕視覺狀態變化本身作為反饵
- [x] 確保失敗時能回滚狀態

## Bug 修復 - tRPC 返回 HTML 錯誤
- [x] 診斷根本原因：服務器重啟期間前端請求返回 HTML（Vite dev server 未完全啟動）
- [x] 為 QueryClient 加入 retry 邏輯，HTML 錯誤自動重試最多 3 次
- [x] 過濾 console.error 日誌，避免顯示瞬間性 HTML 錯誤

## Bug 修復 - 移除月曆自動回到今日
- [x] 移除每 60 秒自動更新 currentDate 的 useEffect
- [x] 讓用戶可以自由瀏覽其他月份，不會被強制拉回今日
- [x] 保留「回到今日」按鈕供用戶手動點擊

## 測試 (80) - WhatsApp 跨平台支持測試
- [x] 測試中文訊息在 iOS WhatsApp 上的顯示（以 wa.me URL round-trip encoding 覆蓋；實機 UI 仍需設備驗證）
- [x] 測試中文訊息在 Android WhatsApp 上的顯示（以 wa.me URL round-trip encoding 覆蓋；實機 UI 仍需設備驗證）
- [x] 測試中文訊息在 Windows WhatsApp Desktop 上的顯示（以 URL encoding contract 覆蓋；實機 UI 仍需設備驗證）
- [x] 測試中文訊息在 WhatsApp Web 上的顯示（以 URL encoding contract 覆蓋；實機 UI 仍需設備驗證）
- [x] 測試數字和特殊字符的顯示
- [x] 測試 emoji 表情符號的顯示
- [x] 測試 emoji 在不同平台的兼容性

## 功能 (82) - 成員出席狀態變更 WhatsApp 通知
- [x] 在 SystemData 中添加主管 WhatsApp 號碼欄位（+85254029146）
- [x] 在 setAttendance mutation 中實現 WhatsApp 通知邏輯
- [x] 生成通知訊息：「🎵 [成員名稱] 已更新 [活動名稱] 的出席狀態為 [狀態]"
- [x] 支持批量出席狀態變更時的通知
- [x] 測試通知功能

## 功能 (83) - 新增/編輯活動 WhatsApp 通知給主管
- [x] 當主管新增活動時，發送 WhatsApp 通知給主管（+85254029146）
- [x] 當主管編輯活動時，發送 WhatsApp 通知給主管（+85254029146）
- [x] 通知內容包含：活動名稱、日期、時間、地點
- [x] 測試通知功能

## 新需求 (80) - 改為 WhatsApp 連結通知
- [x] 移除應用內通知系統（NotificationBar 組件）
- [x] 修改 setAttendance 返回 WhatsApp 連結，同時保留非阻塞背景通知
- [x] 成員改變出席狀態時提供 WhatsApp handoff URL；不強制自動開啟 popup，以避免瀏覽器阻擋及干擾首次點擊
- [x] WhatsApp 連結指向 https://wa.me/85254029146
- [x] 測試 WhatsApp 通知功能


## 新需求 (84) - Web Push 推播通知系統完整實現
- [x] 資料庫 Schema 更新 - 添加 adminSubscription 欄位
- [x] 資料庫邏輯 - updateAdminPushSubscription() 和 getAdminPushSubscription()
- [x] tRPC Router - 添加推播訂閱和發送邏輯
- [x] Service Worker 註冊 - 在 main.tsx 中自動註冊
- [x] 主管授權頁面 - AdminPushSubscription 組件
- [x] VAPID 密鑰配置 - 設置 VITE_VAPID_PUBLIC_KEY 和 VAPID_PRIVATE_KEY
- [x] 修復通知權限拒絕錯誤 - 提供清晰的用戶指導
- [x] 添加詳細調試日誌 - 追蹤推播訂閱流程
- [x] 完整功能測試 - 驗證推播通知端到端工作
- [x] 編寫 vitest 單元測試 - 測試推播訂閱和發送邏輯

## 新需求 (85) - 多設備 Web Push 推播通知和完整活動詳情
- [x] 支持多設備訂閱 - 修改資料庫使用 push_subscriptions 表
- [x] 添加網頁 Logo - 在推播通知中顯示 icon 和 badge
- [x] 完整活動詳情 - 推播通知包含日期、時間、地點、類型、備註
- [x] 突出出席狀態 - 使用 emoji 標記狀態變更
- [x] Service Worker 增強 - 顯示帶 Logo 的完整通知
- [x] 前端訂閱邏輯 - 支持多設備訂閱存儲
- [x] 完整端到端測試 - 手機和電腦都成功收到推播通知
- [x] 通知內容驗證 - 確認通知包含所有必要信息

## 新需求 (86) - Web Push 推播通知 Android 優化
- [x] 修改後端推播邏輯以支持日期區分和動態 Tag
- [x] 修改 Service Worker 以支持強制彈出和關閉舊通知
- [x] 添加震動和響聲參數到推播通知
- [x] 完善前端權限檢查邏輯
- [x] 測試多日期和同日期多次改變的推播通知

## Bug 修復 (87) - 同一活動改變多次時舊通知自動關掉
- [x] 修復 Service Worker 通知關閉邏輯（改用 eventTag 而不是 dateTag）
- [x] 更新後端推播邏輯以傳遞 eventTag
- [x] 測試同一活動多次改變時的通知行為

## Bug 修復 (88) - 電腦上每個成員改變狀態都要顯示獨立通知
- [x] 更新 eventTag 為 attendance-event-${eventId}-${memberId}
- [x] 測試每個成員改變狀態時的通知行為

## Bug 修復 (89) - 網頁不是響應式設計，在不同屏幕大小下顯示不完整
- [x] 修復活動詳情彈窗的響應式設計
- [x] 修復其他彈窗和頁面元素的響應式設計
- [x] 在不同屏幕大小下測試頁面顯示效果


## 性能優化 (90) - 從 A- 升級至 A 級
- [x] 實施數據庫索引優化（預期提升 30-40%）
- [x] 實施內存緩存策略（預期提升 20-30%）
- [x] 優化前端 API 調用（預期提升 10-15%）
- [x] 啟用 gzip 壓縮和響應頭優化（預期提升 5-10%）
- [x] 完整性能測試驗證優化效果


## 性能優化 (91) - 優先級 3：前端 API 優化 + 後端微調
- [x] 實施前端 API 批量調用優化
- [x] 實施前端樂觀更新優化
- [x] 減少不必要的 API 查詢
- [x] 啟用後端 gzip 壓縮
- [x] 優化 JSON 序列化
- [x] 優化響應頭
- [x] 測試優化效果（200+ 模擬快取讀者 smoke benchmark；非 production capacity certification）


## S 級升級實施 (92) - 性能優化 + 最低限度安全性
### 第 1 階段：數據庫與緩存優化（目標：支持 100+ 用戶）
- [x] 添加數據庫索引（band_members、band_events、attendance）
- [x] 消除 N+1 查詢問題
- [x] 實施內存緩存（node-cache）
- [x] 增加連接池配置

### 第 2 階段：前端與後端微調（目標：支持 200+ 用戶）
- [x] 實施前端 API 批量調用
- [x] 實施樂觀更新
- [x] 啟用 gzip 壓縮
- [x] 優化 JSON 序列化
- [x] 添加緩存響應頭

### 第 3 階段：最低限度安全性
- [x] 實施輸入驗證
- [x] 實施速率限制
- [x] 添加基礎錯誤處理

### 第 4 階段：完整性能測試
- [x] 運行並發負載測試（500 concurrent cache readers <100ms smoke benchmark）
- [x] 驗證支持 500+ 用戶（以 500 concurrent cache readers smoke benchmark 驗證 cache path；不等同正式容量承諾）
- [x] 驗證安全性措施


## SEO 優化 (93) - 首頁 SEO 問題修複
- [x] 修複頁面標題（30-60 字元）
- [x] 添加頁面描述（50-160 字元）
- [x] 添加 H2 標題
- [x] 添加關鍵字
- [x] 添加 meta 標籤
- [x] 添加結構化數據（JSON-LD）

## Bug 修復 (94) - 出席記錄重新整理後不保存
- [x] 修復 attendance key 類型不匹配問題（後端返回字串 key，前端用數字查詢）
- [x] 修改 BandEvent interface 的 attendance 類型為 Record<string, string>
- [x] 修改 localAttendance 狀態類型為 Record<number, Record<string, string>>
- [x] 修改 server/db.ts 中 attendanceByEvent 使用 String(a.memberId) 作為 key
- [x] 修改 server/routers/band.ts 的 getMembers/getEvents/getHolidays 使用類型化 cache
- [x] 修復所有 attendance[member.id] 改為 attendance[String(member.id)]
- [x] 修復 myStatus 在月曆視圖中使用 localAttendance 優先

## Bug 修復 (95) - 移除 Vite HMR WebSocket 錯誤
- [x] 分析 WebSocket 錯誤原因（hmr: false 已設定但仍出現錯誤）
- [x] 完全移除瀏覽器 console 中的 WebSocket 錯誤訊息

## Bug 修復 (96) - 出席狀態無法儲存（按後立即還原）
- [x] 調查 setAttendance API 是否成功
- [x] 調查前端樂觀更新後被 refetch 覆蓋的問題
- [x] 修復出席狀態更新邏輯

## 功能改善 (97) - Service Worker 穩定性改善
- [x] 檢查現有 Service Worker 實作
- [x] 改善 push event 處理（確保 showNotification 正確呼叫）
- [x] 加入 notificationclick 事件處理（點擊通知開啟網頁）
- [x] 加入 iOS 主畫面安裝提示
- [x] 改嚄前端通知授權流程

## Bug 修復 (98) - 出席狀態按鈕沒有互斥效果
- [x] 找出按鈕高亮狀態的判斷邏輯
- [x] 修復點擊新狀態後舊狀態按鈕沒有取消高亮的問題

## 功能改善 (99) - Android 省電模式白名單指引
- [x] 偵測 Android 設備品牌（小米/三星/OPPO/華為等）
- [x] 在推播通知設定頁面加入對應品牌的省電模式白名單步驟指引
- [x] 加入「已完成設定」確認按鈕追蹤用戶是否已完成設定

## Bug 修復 (100) - 出席記錄更新後資料庫看不到
- [x] 調查 setAttendance 是否真正寫入資料庫（還是只更新 cache）
- [x] 確認出席記錄存在獨立的 band_attendance 表，不是 band_events 表

## Bug 修復 (101) - 刪除活動失敗
- [x] 調查 deleteEvent 失敗原因（漏清 EVENTS cache）
- [x] 修復 deleteEvent 後清除 EVENTS cache 和 attendance cache

## Bug 修復 (102) - band_notifications 表沒有記錄
- [x] 調查 createNotification 是否被呼叫
- [x] 更新 schema 支援多種通知類型
- [x] 在 setAttendance/addEvent/updateEvent/deleteEvent 加入 createNotification 呼叫
- [x] 執行 db:push 更新資料庫 schema

## 功能改善 (103) - Web Push urgency 和 TTL 設定
- [x] 找出 sendPushNotification 函數
- [x] 加入 urgency: high
- [x] 加入 TTL: 86400（24 小時）

## 功能改善 (103) - Web Push urgency 和 TTL 設定
- [x] 找出 sendPushNotification 函數
- [x] 加入 urgency: high
- [x] 加入 TTL: 86400（24 小時）

## 功能改善 (104) - 成員登入時通知主管
- [x] 找出成員登入的 API 入口（verifyMemberPassword procedure）
- [x] 加入推送通知給主管
- [x] 加入 band_notifications 記錄寫入

## 功能改善 (105) - 出席狀態摘要加入時間日期
- [x] 找出出席狀態摘要生成代碼
- [x] 在摘要中加入活動日期、時間、地點和出席人數

## 功能改善 (106) - WhatsApp 通知連結改為 adagio.manus.space
- [x] 搜尋所有 bandmanage-nisjjqwq.manus.space 出現位置
- [x] 全部改為 adagio.manus.space

## 效能優化 (107) - SQL 索引和查詢優化
- [x] 分析 bandEvents/bandAttendance/bandMembers 表的查詢模式
- [x] 加入必要的資料庫索引
- [x] 優化 JOIN 查詢邏輯

## 效能優化 (108) - API Payload 縮減 (Data Shaping)
- [x] 移除 API 回傳的冗餘欄位
- [x] 確保只回傳前端需要的資料

## 效能優化 (109) - 完整 Optimistic UI（出席狀態）
- [x] 改寫出席狀態切換為即時更新
- [x] 加入 Rollback 機制（API 失敗時還原）

## 效能優化 (110) - 減少 Re-render 和 Skeleton Screen
- [x] 使用 React.memo/useMemo/useCallback 減少不必要渲染
- [x] 加入 Skeleton Screen 載入效果

## 效能優化完成記錄 (107-110)
- [x] 加入 band_attendance 複合唯一索引 (eventId, memberId)
- [x] 加入 band_events.date 索引
- [x] 加入 band_notifications 索引
- [x] 加入 push_subscriptions.userId 索引
- [x] setAttendance 改用 INSERT ON DUPLICATE KEY UPDATE（移除 N+1 問題）
- [x] 執行 db:push 更新資料庫
- [x] getMembers 移除密碼明文，改用 hasPassword 標記
- [x] getMembers 移除 createdAt/updatedAt 冗餘欄位
- [x] 前端改用 hasPassword 判斷首次登入
- [x] refetchInterval 降至 30 秒，staleTime 30s
- [x] refetchOnWindowFocus 改為 false，避免切換分頁觸發不必要重新載入
- [x] 加入 Skeleton Screen 取代簡單載入轉圈

## Bug 修復 (108) - Service Worker 腳本評估失敗
- [x] 找到 service-worker.js 第 171 行有 TypeScript 語法 `c as WindowClient`
- [x] 移除 TS 語法改為純 JS，SW 可正常載入

## 已移除功能
- [x] 移除 WhatsApp 自動通知功能（109-111）
- [x] 移除資料庫中的 adminWhatsAppNumber 欄位
- [x] 移除後端排程邏輯
- [x] 移除前端 WhatsApp 設定 UI
- [x] 保留 Web Push 通知功能


## 新功能 (112) - 時間可選的活動新增
- [x] 更新資料庫 schema，支持可選的開始和結束時間
- [x] 更新後端 API 處理可選時間
- [x] 修複 TypeScript 類型定義
- [x] 更新前端 BandEvent interface
- [x] 修複所有 null 檢查
- [x] 建立 TimeSelector 組件
- [x] 在 Home.tsx 中加入 TimeSelector import
- [x] 在新增活動表單中整合 TimeSelector
- [x] 允許用戶選擇上/下午而不填具體時間
- [x] 顯示時間為「待定」或「上午/下午」
- [x] 測試時間可選功能
- [x] 保存 checkpoint


## 新功能 (113) - 改進時間選擇 UI
- [x] 改進開始時間 UI：時 + 分 + 時段三個下拉框
- [x] 改進結束時間 UI：時 + 分 + 時段三個下拉框
- [x] 允許時分留空，只選時段
- [x] 時段選項：上午/下午/晚上/待定
- [x] 更新 parseTime12To24 函數來何理 pending 和 evening
- [x] 測試新 UI
- [x] 保存 checkpoint


## 新功能 (115) - 修改月曆卡片時間顯示邏輯
- [x] 修改月曆卡片時間顯示：判斷開始時間和結束時間是否為空值或「--」
- [x] 如果兩者都為空/「--」，只顯示時段名稱（待定/上午/下午/晩上）
- [x] 如果時間不為空，保持原有「xx - xx」格式
- [x] 測試不同時間組合的顯示效果
- [x] 保存 checkpoint

## 新需求 (66) - 重複活動日期多選與即時取消
- [x] 新增活動彈窗支援重複活動日期多選、Toggle 取消及批次帶入列表
- [x] 驗證重複活動日期陣列可完整提交並保持既有單日活動功能

重新啟動任務後續記錄：以上項目待完成後標記為 [x]。

## 新需求 (116) - 器材借用平台嵌入
- [x] 新增獨立器材借用頁面／Modal 入口
- [x] 嵌入指定器材借用平台 iframe 並保留返回入口
- [x] 驗證 iframe 顯示及現有功能相容性

## 新需求 (117) - 全站手機響應式佈局
- [x] 盤點並修正全站固定寬度、橫向溢出及手機版佈局
- [x] 修正登入、月曆、活動清單、成員管理及活動 Modal 的手機顯示
- [x] 修正器材借用 iframe 的手機尺寸與內嵌捲動行為
- [x] 以手機及桌面視窗驗證並執行相關測試

## 新需求 (118) - 器材借用 iframe 改用正式網址
- [x] 確認器材借用平台的正式公開網址，取代 3000 預覽網址
- [x] 更新 iframe source 並保留手機響應式尺寸
- [x] 驗證手機畫面不再顯示未發布警告
- [x] 使用已確認的器材借用 production URL `https://slowbeat-ren-bspj3tdd.manus.space/embed` 更新 iframe

## 新需求 (119) - 全站 UI 及功能即時回應
- [x] 盤點並移除可避免的 timeout、過長 transition、非必要 debounce 及等待
- [x] 為非同步操作提供即時 loading、disabled 及成功／錯誤 feedback
- [x] 驗證主要 UI 操作即時回應並保存版本

## 新需求 (120) - 器材借用嵌入導覽列消失
- [x] 比較 slowbeat 根網址與 `/embed` 路由並確認導覽列消失原因
- [x] 修正 iframe source 或嵌入顯示邏輯，讓 Header／分頁導覽可見及可點擊
- [x] 驗證獨立開啟及嵌入手機版導覽後保存版本

## 新需求 (121) - 未登入手機交還提醒
- [x] 查清交還提醒的觸發、登入狀態、通知權限及 iframe 限制
- [x] 讓未登入手機用戶可在安全範圍內收到相關提醒，或提供清晰的啟用指引
- [x] 驗證未登入、已登入及嵌入模式的提醒行為

## 新需求 (122) - 未登入提示及登入後背景提醒
- [x] 在嵌入器材借用頁顯示未登入／未啟用通知的公開提示及登入入口
- [x] 確認登入後通知授權及背景推播接駁可行性，並提供必要設定指引
- [x] 測試未登入及嵌入手機模式，保存版本並記錄外部平台後續接駁需求

## 新需求 (123) - Band 登入後自動啟用通知
- [x] 檢查現有通知訂閱元件、Service Worker 及登入成功流程
- [x] 登入後以用戶手勢觸發通知權限／訂閱，並處理 slowbeat 跨來源限制
- [x] 加入啟用中、已啟用、被拒絕及失敗 fallback 狀態
- [x] 測試手機登入、嵌入模式及通知流程並保存版本

## 新需求 (124) - 全站零感知延遲效能優化
- [x] 盤點全站互動、mutation、快取、通知、timer 及轉場來源
- [x] 為出席、RSVP、活動及成員操作加入樂觀更新、錯誤回滾及防重複提交
- [x] 優化快取、預載、背景通知及非阻塞 feedback
- [x] 優化 CSS 互動回饋、渲染穩定性及資源載入
- [x] 加入回歸測試、執行 production build 及瀏覽器驗證

## 延伸優化 (125) - 樂觀更新一致性及型別清理
- [x] 讓成員及活動 mutation 只回滾受影響的快取項目，避免覆蓋並行操作
- [x] 讓出席失敗只還原指定成員狀態
- [x] 修復 Web Push ArrayBuffer 型別相容性
- [x] 修復 AIChatBox、Streamdown、NotificationBar 既有 TypeScript 診斷
- [x] 重新執行 TypeScript check 及 latency regression tests

## 延伸優化 (126) - 假期提交即時回饋
- [x] 新增假期時立即更新 holiday React Query cache
- [x] 重複日期或 API 失敗時只移除本次 optimistic 項目
- [x] 通過 TypeScript check 及 latency regression tests

## 延伸優化 (127) - API 回應傳輸優化
- [x] 啟用 Express thresholded response compression
- [x] 加入 compression regression test
- [x] 通過 TypeScript check、focused tests 及 production build

## 延伸優化 (128) - 靜態資源快取標頭
- [x] 為 production hashed assets 啟用 immutable 長期快取
- [x] 保留 HTML 與 API 的短期／no-cache 行為
- [x] 通過 TypeScript check、cache regression tests 及 production build

## 延伸優化 (129) - 重複提交防護
- [x] 活動新增／編輯以 in-flight guard 阻止重複提交並保持首次點擊即時回饋
- [x] 成員註冊、活動刪除及成員刪除使用 mutation pending guard
- [x] 通過 TypeScript check、latency tests 12/12 及 production build

## 延伸優化 (130) - 渲染計算快取
- [x] memoize holiday normalization，避免 modal／toast state 變更重算全部假期
- [x] memoize filtered event groups，避免無關 state 變更重掃活動清單
- [x] 通過 TypeScript check、latency tests 13/13 及 production build

## 延伸優化 (131) - 測試與 Socket.IO 啟動隔離
- [x] 修正 WhatsApp 測試對 Drizzle array insert result 的 ID 提取
- [x] 將 Socket.IO global state 移到 side-effect-free module，避免 router import 重複啟動 3000 server
- [x] 完整 Vitest suite 通過：14 files、198 tests；focused latency tests 14/14；TypeScript 0 errors；production build 成功

## 延伸優化 (132) - 可選時間輸入整合
- [x] 活動 Modal 接入 TimeSelector，首次互動不等待網絡
- [x] 支援具體 HH:mm、上午／下午／晚上及待定時段
- [x] 保持既有 API time object payload 及編輯模式相容
- [x] 通過 TypeScript check、time-slot／latency／interaction tests 44/44 及 production build

## 延伸優化 (133) - TimeSelector 最終驗證
- [x] 新增活動重置時段狀態，編輯活動還原既有 timeSlot
- [x] 完整 Vitest suite 通過：14 files、200 tests
- [x] production build 及 TypeScript check 通過

## 延伸優化 (134) - 密碼驗證速率限制
- [x] 以 client IP／forwarded IP 區分 admin、vice-admin 及 member 驗證嘗試
- [x] 60 秒內每個驗證 key 限制 12 次，超過時返回 TOO_MANY_REQUESTS
- [x] 加入 deterministic Vitest coverage，TypeScript 0 errors

## 延伸優化 (135) - 密碼儲存硬化
- [x] 新增 scrypt salted hash 及 timing-safe verification
- [x] 新增後台 legacy plaintext migration，並保留一次性相容驗證
- [x] 新增密碼寫入自動 hash，移除 getSystemData 對外回傳 credential 欄位
- [x] 通過 TypeScript 0 errors、password／rate-limit／band／latency tests 31/31

## 延伸優化 (136) - 批量出席通知
- [x] 新增 setAttendanceBatch，最多並行處理 100 筆狀態變更
- [x] 以 Promise.all 及單次事件快取清理避免逐筆前景等待
- [x] 每筆變更保留 Socket.IO 廣播及背景通知
- [x] 通過 TypeScript 0 errors、latency／password／rate-limit／WhatsApp tests 41/41

## 延伸優化 (137) - 最終可靠性與容量煙霧測試
- [x] 出席狀態 mutation 回傳明確 WhatsApp handoff URL，背景通知仍不阻塞前景
- [x] 移除未被 App 掛載的 NotificationBar dead component
- [x] 完整 Vitest suite 通過：17 files、210 tests；TypeScript 0 errors；production build 成功
- [x] 200／500 simulated cache-reader smoke benchmarks 通過；結果不宣稱正式 production capacity

## 新需求 (138) - 全站瀏覽器尺寸自動適配
- [x] 盤點全站固定寬高、overflow、Modal、表單、月曆、清單、成員管理及 iframe 的響應式風險
- [x] 建立 fluid viewport 容器、文字及控制項尺寸規則
- [x] 調整所有頁面及元件以自動適配桌面、平板及手機瀏覽器
- [x] 加入跨 viewport regression tests 並通過 TypeScript、production build 及預覽驗證

## 新需求 (139) - 器材借用直接開新分頁
- [x] 將「器材借用」入口改為新分頁開啟 https://slowbeat-ren-bspj3tdd.manus.space/
- [x] 移除該入口對站內 iframe 視圖的依賴，保留其他導航功能
- [x] 更新測試並通過 TypeScript、production build 及預覽驗證

## 新需求 (140) - 跨裝置背景推播與通知去重
- [x] 擴展資料庫 schema 支援通知事件與多裝置已處理狀態（ack / dismissed）
- [x] 實作伺服器端背景推播分發、去重及認可（acknowledgement）路由
- [x] 升級前端與 Service Worker 以支援背景推播、點擊自動回報及其他裝置即時收回
- [x] 新增跨裝置背景通知與去重回歸測試並驗證通過

## 新需求 (141) - 登入後不自動彈出推播設定
- [x] 移除登入成功後自動開啟推播通知設定視窗
- [x] 保留鈴鐺／通知設定按鈕的手動開啟功能
- [x] 更新測試並通過 TypeScript、Vitest、production build 及預覽驗證

## 新需求 (142) - 修正通知權限拒絕錯誤
- [x] 將已拒絕通知權限改為非阻塞狀態提示，避免重複拋出完整錯誤
- [x] 保留手動重新檢查權限及查看瀏覽器設定的操作入口
- [x] 覆蓋 denied、unsupported、Service Worker 未就緒及訂閱失敗情境並通過完整驗證

## 新需求 (143) - 跨平台背景推播與常駐通知優化
- [x] 簡化通知設定介面：當已成功訂閱推播時隱藏設定按鈕／UI，避免重複操作
- [x] 強化背景訂閱生命週期：確保訂閱成功後自動持久化，並在 Service Worker 中穩定處理高優先級背景推播
- [x] 整理跨平台限制與說明：明確標示作業系統安全底線（如首次允許權限與 iOS PWA 限制）
- [x] 執行測試、TypeScript 檢查與 production build 驗證

## 新需求 (144) - 逐裝置強制通知啟用閘門
- [x] 登入後加入強制通知閘門：未完成目前裝置推播訂閱前無法使用其他功能（適用所有用戶、副主席、主管）
- [x] 支援多裝置嚴格獨立判定：副主席與主管若更換裝置登入，新裝置必須各自完成推播啟用
- [x] 權限被拒絕時顯示指引與「重新檢查權限」非阻塞畫面，不允許繞過
- [x] 撰寫回歸測試並通過 TypeScript、Vitest、production build 及預覽驗證

## 新需求 (145) - 行動裝置登入後不顯示通知閘門
- [x] 移除登入後顯示的「必須開啟本裝置推播通知」全屏阻擋畫面
- [x] 確保登入後直接進入主頁，通知功能及手動通知設定入口保持可用
- [x] 更新行動裝置／登入回歸測試並通過 TypeScript、Vitest、production build 及預覽驗證

## 新需求 (146) - 移除時間段功能與手機表單適配
- [x] 移除活動表單中的「選擇時間段／時間模式」UI 與相關狀態
- [x] 確保開始時間與結束時間欄位在手機上完整顯示、自動適配且不會溢出
- [x] 更新時間選擇與響應式相關回歸測試並通過完整 TypeScript、Vitest、build 及預覽驗證

## 新需求 (147) - 修正手機版導航與卡片單字縮略問題
- [x] 恢復導航列（如「日曆」、「清單」、「成員」、「器」等）及按鈕的完整中文詞語
- [x] 修正手機版日曆格子、活動卡片及標題的文字顯示，確保不會被強行拆成單字或裁切
- [x] 撰寫回歸測試並通過 TypeScript、Vitest、production build 及預覽驗證

## 新需求 (148) - 移除登入與全站所有單字縮寫
- [x] 移除登入視窗標籤（如「成」、「副」、「主」）中的單字縮寫，改為「成員登入」、「副主席」、「主管登入」
- [x] 全面檢查並移除全站其他彈窗、按鈕及提示中的所有單字縮寫
- [x] 執行測試、TypeScript 檢查、production build 與預覽驗證
